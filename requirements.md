...
GitHub Actions (cron: 毎朝 5:00 JST)
  ...
  │     Zenn, OpenAI Blog, Anthropic Blog, HackerNews, GitHub Trending ...
  │
...
| # | ソース | 取得方法 | エンドポイント / 備考 |
|---|--------|----------|----------------------|
| 1 | **Zenn** | RSS | `https://zenn.dev/feed` トピック別: `https://zenn.dev/topics/nextjs/feed` 等 |
| 2 | **Zenn トレンド** | Firecrawl | `https://zenn.dev` トップのトレンド記事をスクレイピング |
| 3 | **OpenAI Blog** | Firecrawl | `https://openai.com/blog` |
| 4 | **Anthropic Blog** | Firecrawl | `https://www.anthropic.com/news` |
| 5 | **Google AI Blog** | Firecrawl / RSS | `https://blog.google/technology/ai/` |
| 6 | **Hacker News** | API（無料） | `https://hacker-news.firebaseio.com/v0/topstories.json` AI関連をフィルタ |
| 7 | **GitHub Trending** | Firecrawl | `https://github.com/trending?since=daily` 言語フィルタ可 |

### X (Twitter) API について

Basic プランで月 $200 のため **初期スコープから除外**。Zenn + HackerNews + 各社ブログで十分カバーできる。必要になったら拡張ポイントから追加する。
...
type CollectedArticle = {
  source: string;       // "zenn" | "openai" | "anthropic" | "google" | "hackernews" | "github"
  title: string;
  url: string;
...
│   ├── collectors/
│   │   ├── zenn.ts                # Zenn RSS
│   │   ├── openai-blog.ts        # OpenAI Blog (Firecrawl)
│   │   ├── anthropic-blog.ts     # Anthropic Blog (Firecrawl)
│   │   ├── google-ai-blog.ts     # Google AI Blog
│   │   ├── hackernews.ts         # HN API
│   │   ├── github-trending.ts    # GitHub Trending (Firecrawl)
│   │   └── index.ts              # 全collector統合
│   ├── ai/
│   │   ├── summarize.ts          # Claude API 要約・優先度付け
│   │   └── prompts.ts            # プロンプトテンプレート
│   ├── slack/
│   │   └── post.ts               # Slack Webhook 投稿
│   ├── config/
│   │   └── profile.yaml          # ユーザープロフィール
│   └── types.ts                  # 型定義
├── data/
